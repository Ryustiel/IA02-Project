...
def scaner(hitman: HitmanReferee):
    vue=hitman.__get_vision(3)
    for i in range(len(vision)):
        var=case_to_variable(vision[i][0], vision[i][1])
        if [var] not in dimacsClauses:
            dimacsClauses.append([var])
            carte[vision[i][0]]=vision[i][1]
            for j in range (var-vision[i][1]+1,var-vision[i][1]+1+13):#si il y a une chose sur une case, il n'y a pas ce qui n'est pas cette chose
                if j !=val and [-j] not in dimacsClauses:
                    dimacsClauses.append([-j])
